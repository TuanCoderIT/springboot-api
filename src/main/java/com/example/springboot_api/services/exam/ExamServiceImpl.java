...
public class ExamServiceImpl implements ExamService {
    ...
    private final AsyncQuestionGenerationService asyncQuestionGenerationService;
    ...
    public ExamResponse generateQuestions(UUID examId, GenerateQuestionsRequest request, UUID lecturerId) {
        ...
        if (exam.getStatus() != ExamStatus.DRAFT && exam.getStatus() != ExamStatus.GENERATING) {
        ...
        try {
            ...
            asyncQuestionGenerationService.generateQuestionsAsync(exam, notebookFiles, request);

            log.info("Triggered ASYNC question generation for exam {}", examId);
            return mapToExamResponse(exam, lecturerId);

        } catch (Exception e) {
            log.error("Error triggering question generation for exam {}: {}", examId, e.getMessage(), e);
    ...
    public ExamAttemptResponse startExam(UUID examId, StartExamRequest request, String studentCode) {
        ...
        // Fetch exam with questions only (options will be fetched separately when
        // needed)
        ...
        Optional<ExamAttempt> existingAttempt = examAttemptRepository.findCurrentAttemptByStudentCode(examId,
                studentCode);
    ...
    private ExamAttemptResponse mapToExamAttemptResponse(ExamAttempt attempt) {
        ...
        if (attempt.isInProgress() && attempt.getStartedAt() != null) {
            int elapsedMinutes = (int) java.time.Duration.between(attempt.getStartedAt(), LocalDateTime.now())
                    .toMinutes();
    ...
    private ExamAttemptResponse.ExamQuestionResponse.QuestionOptionResponse mapSnapshotToOptionResponse(
            Map<String, Object> optionMap) {
        ExamAttemptResponse.ExamQuestionResponse.QuestionOptionResponse option = new ExamAttemptResponse.ExamQuestionResponse.QuestionOptionResponse();
    ...
    private ExamResultResponse mapToExamResultResponse(ExamAttempt attempt) {
        ...
        response.setIncorrectAnswers(
                (int) answers.stream().filter(a -> Boolean.FALSE.equals(a.getIsCorrect())).count());
